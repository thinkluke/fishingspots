<?php
/*
Plugin Name: Geo Data Store farts
...
if ( ! class_exists( 'sc_GeoDataStore' ) )
{
	class sc_GeoDataStore
    {
		...
		*/
		...
		private static function checkDBVersion()
        {
            ...
            $sql = "CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . self::$tablename . "` (
						  ...
						  `spot_cats` varchar(255) DEFAULT NULL,
						  `spot_tags` varchar(255) DEFAULT NULL,
		...
		*/
		...
		/*
		public static function after_post_terms( $term_id, $name, $slug )
        {
			self::meta_data_captured( $term_id, "add", $name, $slug );
		}
		...
		*/
		...
		private static function meta_data_captured( $meta_id, $post_id,  $meta_key, $action = "add", $meta_value = 0 )
        {
			global $wpdb;
            ...
            if($action == "add")
            {
                ...
                $wpdb->query( "
					...
					(`post_id`, `meta_id`, `post_type`, `lat`, `lng`, `title`, `post_content`, `spot_cats`, `spot_tags`, `fish_species`, `regions`, `water_type`, `likes` ) 
					VALUES (".(int) $post_id.", ".(int) $meta_ids.", '{$post_type}', '".(float) $coords[0]."', '".(float) $coords[1]."') 
		...
		public static function reindex()
        {
            ...
            $sql .= "SELECT
					...
					,(SELECT group_concat({$wpdb->terms}.name separator ', ') 
						FROM {$wpdb->terms}
						INNER JOIN wp_term_taxonomy on {$wpdb->terms}.term_id = wp_term_taxonomy.term_id
						INNER JOIN wp_term_relationships wpr on wpr.term_taxonomy_id = wp_term_taxonomy.term_taxonomy_id
						WHERE taxonomy= 'spot_tags' and wp_posts.ID = wpr.object_id
					) AS 'spot_cats'
					,(SELECT group_concat({$wpdb->terms}.name separator ', ') 
						FROM {$wpdb->terms}
						INNER JOIN wp_term_taxonomy on {$wpdb->terms}.term_id = wp_term_taxonomy.term_id
					...
					,(SELECT group_concat({$wpdb->terms}.name separator ', ') 
						...
						INNER JOIN wp_term_taxonomy on {$wpdb->terms}.term_id = wp_term_taxonomy.term_id
						INNER JOIN wp_term_relationships wpr on wpr.term_taxonomy_id = wp_term_taxonomy.term_taxonomy_id
						WHERE taxonomy= 'region' and wp_posts.ID = wpr.object_id
					) AS 'regions'
					,(SELECT group_concat({$wpdb->terms}.name separator ', ') 
						FROM {$wpdb->terms}
						INNER JOIN wp_term_taxonomy on {$wpdb->terms}.term_id = wp_term_taxonomy.term_id
						INNER JOIN wp_term_relationships wpr on wpr.term_taxonomy_id = wp_term_taxonomy.term_taxonomy_id
						WHERE taxonomy= 'water_type' and wp_posts.ID = wpr.object_id
					) AS 'water_type'
					,(SELECT group_concat({$wpdb->terms}.name separator ', ') 
						FROM {$wpdb->terms}
						INNER JOIN wp_term_taxonomy on {$wpdb->terms}.term_id = wp_term_taxonomy.term_id
						INNER JOIN wp_term_relationships wpr on wpr.term_taxonomy_id = wp_term_taxonomy.term_taxonomy_id
						WHERE taxonomy = 'fish_species' and wp_posts.ID = wpr.object_id
					) AS 'fish_species'
					
					,(SELECT {$wpdb->postmeta}.meta_value
						FROM {$wpdb->postmeta}
						
						WHERE meta_key = 'fishingspots_likes' and wp_posts.ID = wp_postmeta.post_id
					) AS 'likes'
